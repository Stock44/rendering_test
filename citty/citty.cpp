...
int main(int argc, char *argv[]) {
  ...
  //    auto cameraStore =
  //    componentManager.getComponentStore<graphics::Camera>(); auto targetStore
  //    = componentManager.getComponentStore<traffic::PathfindTarget>();
  //
  ...
  //    map::loadXMLMap("/home/hiram/Projects/citty/samples/sample_map.osm",
  //    componentManager, entityManager); std::cout << "Number of entities in
  //    the system: " << entityManager.createEntity() << std::endl; std::cout <<
  //    "Number of roads: " << roadStore->getComponents().size() << std::endl;
  //    std::cout << "Number of nodes: " << nodeStore->getComponents().size() <<
  //    std::endl;
  ...
  std::jthread engineThread([&engine, &renderingSystem, &entityStore,
    ...
    graphics::Mesh cubeMesh{
        ...
        {
            //                                indices
            0, 1, 3, 0, 3, 2, 2, 3, 7, 2, 7, 6, 4, 0, 2, 4, 2, 6,
            1, 5, 7, 1, 7, 3, 4, 5, 1, 4, 1, 0, 5, 4, 6, 5, 6, 7,
        }};
    ...
    testEntity.addComponent<engine::Transform>(
        Eigen::Quaternionf::Identity(), Eigen::Vector3f{10.0f, 0.0f, 0.0f});
    ...
    testEntity2.addComponent<engine::Transform>(
        Eigen::Quaternionf::Identity(), Eigen::Vector3f{0.0f, 0.0f, -5.0f},
        Eigen::Vector3f{1.0f, 1.0f, 1.0f}, testEntity);
    testEntity2.addComponent<graphics::Graphics>(pyramidMeshId, testMaterialId);

    auto testEntity3 =
        engine::Entity{entityStore.newEntityId(), componentStore};
    testEntity3.addComponent<engine::Transform>(
        Eigen::Quaternionf::Identity(), Eigen::Vector3f{0.0f, 2.0f, 0.0f},
        Eigen::Vector3f{1.0f, 1.0f, 1.0f}, testEntity2);
    ...
    modelEntity.getComponent<engine::Transform>().position = {10.0f, -3.0f,
                                                              0.0f};
    ...
    pointLightEntity.addComponent<graphics::PointLight>(
        Eigen::Vector3f(1.0f, 1.0f, 1.0f), 20.0f);
    ...
    while (!stopToken.stop_requested()) {
      ...
      testEntity.getComponent<engine::Transform>().rotation *=
          Eigen::Quaternionf(Eigen::AngleAxisf(deltaTime * 0.001f, rotation));
    }
  });
