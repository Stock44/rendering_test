...
Renderer::Renderer() {
  ...
  lightAccumulationBuilder.addShader(
      {"shaders/lightAccumulation.vsh", ShaderType::VERTEX});
  lightAccumulationBuilder.addShader(
      {"shaders/lightAccumulation.fsh", ShaderType::FRAGMENT});
  ...
  lightCullingBuilder.addShader(
      {"shaders/lightCulling.csh", ShaderType::COMPUTE});
  lightCullingShaderProgram = lightCullingBuilder.build();

  vertexBuffer = std::make_shared<Buffer<Vertex>>(BufferUsage::STATIC_DRAW);
  indexBuffer =
      std::make_shared<Buffer<unsigned int>>(BufferUsage::STATIC_DRAW);
...
Renderer::MeshId Renderer::loadMesh(const Mesh &mesh) {
  auto &meshRecord = meshRecords.emplace_back(
      ...
      indexBuffer->getSize(), mesh.indices.size());
  ...
  vao.configureAttrib(1, vertexBuffer, 3, AttributeType::FLOAT, false,
                      offsetof(Vertex, normal));
  vao.configureAttrib(2, vertexBuffer, 3, AttributeType::FLOAT, false,
                      offsetof(Vertex, tangent));
  vao.configureAttrib(3, vertexBuffer, 3, AttributeType::FLOAT, false,
                      offsetof(Vertex, bitangent));
  vao.configureAttrib(4, vertexBuffer, 2, AttributeType::FLOAT, false,
                      offsetof(Vertex, texCoords));

  vao.setVertexIndicesBuffer(indexBuffer);

  vao.bindBuffer(meshRecord.transformBuffer, 0);
  vao.configureAttrib(5, meshRecord.transformBuffer, 4, AttributeType::FLOAT,
                      false, 0);
  vao.configureAttrib(6, meshRecord.transformBuffer, 4, AttributeType::FLOAT,
                      false, 4 * sizeof(float));
  vao.configureAttrib(7, meshRecord.transformBuffer, 4, AttributeType::FLOAT,
                      false, 8 * sizeof(float));
  vao.configureAttrib(8, meshRecord.transformBuffer, 4, AttributeType::FLOAT,
                      false, 12 * sizeof(float));
...
/**
 * This function updated the render targets' buffer sizes, as well as updating
 * several variables used within the shaders.
...
void Renderer::setViewportDimensions(unsigned int width,
  ...
  lightCullingWorkgroupsX = (width + 15) / 16;
  lightCullingWorkgroupsY = (height + 15) / 16;
  numberOfTiles = lightCullingWorkgroupsX * lightCullingWorkgroupsY;
  visiblePointLightIndexBuffer->reallocate(1024 * numberOfTiles,
                                           BufferUsage::STREAM_DRAW);
  lightCullingShaderProgram.setUniform("screenSize", width, height);

  lightAccumulationShaderProgram.setUniform("numberOfTilesX",
                                            lightCullingWorkgroupsX);

  // initialize depth framebuffer
  depthTexture = std::make_shared<Texture2D>(
      SizedImageFormat::DEPTH_COMPONENT24, width, height);
  ...
  hdrColorTexture =
      std::make_shared<Texture2D>(SizedImageFormat::RGB8, width, height);
  hdrColorTexture->setMinFilter(MinFilter::LINEAR);
  hdrColorTexture->setMagFilter(MagFilter::LINEAR);

  hdrDepthRenderbuffer = std::make_shared<Renderbuffer>(
      SizedImageFormat::DEPTH_COMPONENT24, width, height);
...
void Renderer::render() {
  ...
  visiblePointLightIndexBuffer->bindToTarget(
...
void Renderer::setGraphicsEntities(
  ...
  std::ranges::transform(
      graphicsEntities, std::back_inserter(keyedGraphicsEntities),
      [](GraphicsEntity const &rc) { return std::make_pair(rc.key(), rc); });

  std::ranges::sort(keyedGraphicsEntities, std::less<>(),
                    &std::pair<std::size_t, GraphicsEntity>::first);
  ...
  for (auto &[key, graphicsEntity] : keyedGraphicsEntities) {
    ...
    if (graphicsEntity.materialId != currentMaterial ||
        graphicsEntity.meshId != currentMesh) {
      auto offset = currentMeshRecord->transforms.size() - bucketSize;
      renderCommands.emplace_back(bucketSize, offset, currentMaterial,
                                  currentMesh);
  ...
  for (auto &meshRecord : meshRecords) {
    ...
    if (transformBuffer->getSize() < transforms.size()) {
      transformBuffer->reallocate(transforms.size() * 2,
                                  BufferUsage::STREAM_DRAW);
    }
