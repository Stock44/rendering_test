...
TEST_CASE("path hashing/comparison does not throw on nonexistent paths", "[GLFWRenderingSystem]") {
    ...
    SECTION("comparing nonexistent paths does not throw") {
        ...
        REQUIRE_FALSE(comparison(missing, std::filesystem::path{"other/missing.png"}));
    }
}
